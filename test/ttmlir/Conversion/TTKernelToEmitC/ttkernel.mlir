...
module {
  ...
  module @ttkernel_fpu_operations {
    ...
    func.func @unary_bcast_init() -> () attributes {ttkernel.arg_spec = #ttkernel.arg_spec< ct_args = [<arg_type = cb_port, operand_index = 0>, <arg_type = cb_port, operand_index = 1>]>, ttkernel.thread = #ttkernel.thread<compute>} {
      ...
      // CHECK: %[[IN_CB:.*]] = emitc.literal "get_compile_time_arg_val(0)"
      // CHECK: %[[OUT_CB:.*]] = emitc.literal "get_compile_time_arg_val(1)"
      "ttkernel.unary_bcast_init"(%in_cb, %out_cb) <{bcast_dim = #ttkernel.bcast_dim<bcast_dim_row>}> : (!cb0_tiles, !cb1_tiles) -> ()
      "ttkernel.unary_bcast_init"(%in_cb, %out_cb) <{bcast_dim = #ttkernel.bcast_dim<bcast_dim_col>}> : (!cb0_tiles, !cb1_tiles) -> ()
      "ttkernel.unary_bcast_init"(%in_cb, %out_cb) <{bcast_dim = #ttkernel.bcast_dim<bcast_dim_scalar>}> : (!cb0_tiles, !cb1_tiles) -> ()
      "ttkernel.unary_bcast_init"(%in_cb, %out_cb) <{bcast_dim = #ttkernel.bcast_dim<bcast_dim_none>}> : (!cb0_tiles, !cb1_tiles) -> ()
    ...
    func.func @unary_bcast() -> () attributes {ttkernel.arg_spec = #ttkernel.arg_spec< ct_args = [<arg_type = cb_port, operand_index = 0>]>, ttkernel.thread = #ttkernel.thread<compute>} {
      ...
      // CHECK: %[[DST_INDEX:.*]] = "emitc.constant"
      "ttkernel.unary_bcast"(%in_cb, %in_tile_index, %dst_index) <{bcast_dim = #ttkernel.bcast_dim<bcast_dim_row>}> : (!cb0_tiles, index, index) -> ()
      "ttkernel.unary_bcast"(%in_cb, %in_tile_index, %dst_index) <{bcast_dim = #ttkernel.bcast_dim<bcast_dim_col>}> : (!cb0_tiles, index, index) -> ()
      "ttkernel.unary_bcast"(%in_cb, %in_tile_index, %dst_index) <{bcast_dim = #ttkernel.bcast_dim<bcast_dim_scalar>}> : (!cb0_tiles, index, index) -> ()
      "ttkernel.unary_bcast"(%in_cb, %in_tile_index, %dst_index) <{bcast_dim = #ttkernel.bcast_dim<bcast_dim_none>}> : (!cb0_tiles, index, index) -> ()
  ...
  module @ttkernel_sfpu_operations {
    ...
    func.func @binary_max_tile() -> () attributes {ttkernel.thread = #ttkernel.thread<compute>} {
      ...
      // CHECK: emitc.call_opaque "binary_max_tile"(%[[DST0_INDEX]], %[[DST1_INDEX]])
      "ttkernel.binary_max_tile"(%dst0_index, %dst1_index) : (i32, i32) -> ()
      return
    }
